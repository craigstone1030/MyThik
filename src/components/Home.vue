<template>
    <div>
            ...
            <div class="container">
                ...
                <div class="row text-center">
                    <div class="col-md-10 mx-auto">
                        ...
                        <p class="story-text"><span class="story-bigger-text">L</span>ocated in the endless space that is the Metaverse, you can find 10,000 of these tough as nails, fiercely loyal brothers-in-arms storming around on the POLYGON blockchain.</p>
                    </div>
                </div>
            </div>
        </section>
                        ...
                        <div class="col-md-8 d-flex justify-content-end" v-if="!soldOut">
                            <div class="dwarf-buy-form">
                                ...
                                <div class="d-flex justify-content-between nft-row-wrapper">
                                    <div class="nft-label">MATIC BALANCE</div>
                                    <div class="nft-balance" v-html="balance + ' MATIC'"></div>
                                </div>
        ...
        <!-- Box item 1 modal popup-->
        <div class="portfolio-modal modal fade" id="checkModal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog mx-auto modal-dialog-centered">
                <div class="modal-content">
                    <!-- <div class="close-modal" data-bs-dismiss="modal"><img src="../assets/img/close-icon.svg" alt="Close modal" /></div> -->
                    <div class="container">
                        <div class="row justify-content-center">
                            <div class="col-lg-12 p-0">
                                <div class="modal-body p-0">
                                    <h5 class="text-uppercase bow wallet-modal-title mb-0">CHECK YOUR NETWORK!</h5>
                                    <div v-if="!this.checkProcess">
                                        <div v-for="(user, index) in invitedUsers" :key="index" class="col-lg-12 mb-2 mt-2 mx-auto p-2 wallet-panel justify-content-center">                                                           
                                            <div class="justify-content-center">
                                                <span class="bow wallet-panel-title">
                                                    Level{{index + 1}} * 
                                        ...
                                        <div class="col-lg-12 mb-2 mx-auto p-2 wallet-panel justify-content-center">                                   
                                            <div class="justify-content-center">
                                    ...
                                    <span v-else class="spinner-border spinner-border-lg" role="status" aria-hidden="true"></span>                                   
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <!-- Invite modal popup-->
        <div class="portfolio-modal modal fade" id="inviteModal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog mx-auto modal-dialog-centered">
                <div class="modal-content">
                    <div class="container">
                        <div class="row justify-content-center">
                            <div class="col-lg-12 p-0">
                                <div class="modal-body p-0">
                                    ...
                                    <div class="col-lg-11 mt-3 mb-3 mx-auto p-3 wallet-panel justify-content-center">
                                        <input type="text" class="form-control" v-model="inviteeWalletAddress" placeholder="Wallet address : 0x03C6FcED478cBbC9a4FAB34eF9f40767739D1Ff7">
                                    </div>                                
                                    <button class="btn btn-primary sec-bg prim col-lg-11 btn-radius btn-xl text-uppercase mb-3 bow btn-shadow" @click="submit()" type="button">
                                        SUBMIT
                                    </button>
                                </div>
                            </div>
        ...
        <!-- Set the price modal popup-->
        <div class="portfolio-modal modal fade" ref="setPriceModal" id="setPriceModal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog mx-auto modal-dialog-centered">
                <div class="modal-content">
                    <div class="container">
                        <div class="row justify-content-center">
                            <div class="col-lg-12 p-0">
                                <div class="modal-body p-0">
                                    <h5 class="text-uppercase bow wallet-modal-title">SET THE PRICE</h5>
                                    <div class="col-lg-11 mt-3 mb-3 mx-auto p-3 wallet-panel justify-content-center">
                                        <input type="number" class="form-control" v-model="perPrice">
                                        <span class="ether-text">MATIC</span>
                                    </div>                                
                                    <button type="button" v-if="!setting" class="btn btn-primary sec-bg prim col-lg-11 btn-radius btn-xl text-uppercase mb-3 bow btn-shadow" @click="setPrice()">
                                        SET
                                    </button>
...
export default {
    ...
    methods : {
        ...
        getExactPrice (complexPrice) {
            if (complexPrice == '' || typeof complexPrice == 'undefined') return 0
            return (complexPrice + "").slice(0, 6)
        },
        getPerPrice() {
            if (this.price) {
        ...
        totalCost () {
            ...
            return this.web3.utils.fromWei(this.cost.toString()) + ' MATIC'
        },
        decrease() {
            if (this.maxMinting) {
        ...
        async connect () {
            ...
            if (this.web3) {
                ...
                if (networkId != process.env.VUE_APP_CHAIN_ID) {
                    this.popup('Please connect to '+process.env.VUE_APP_NETWORK_NAME, 'Error')
                    return
                }
        ...
        async getInvitedList () {
            ...
            try {              
                if (!this.contract)  this.popup('Try again after 5 seconds', 'Info')
                await this.contract.methods.getInvitors3LevelOfAccount(this.account).call()
                .then(receipt => {
                    console.log('receipt', receipt)
                    this.checkProcess = false
                    this.invitedUsers = [0, 0, 0]
                    for (let i = 0; i < receipt.length; i++) {                        
                        this.invitedUsers[parseInt(receipt[i]['level']) - 1]++;
                    }
                });
        ...
        async excel () {
            try {
                return await this.contract.methods.getInvitors3LevelOfAccount(this.account).call()
                .then(receipt => {
                    let ret = receipt.map(one => { 
                        return {
                            "invitor" : one.invitor,
                            "invitee" : one.invitee,
                            "inviteeMintCount" : one.inviteeMintCount,
                            "level" : one.level
                        };
                    })
                    return ret                
        ...
        async setPrice () {
            if (this.perPrice == '' || !this.perPrice) {
                this.popup('Please input the price', 'Error')
                return                
            } 
            if (this.perPrice <= 0) {
                this.popup('Please input correct price', 'Error')
                return                
            }  
            this.setting = true
            try { 
                let sendPrice = (this.perPrice * 10 ** 18).toString()
                let fee = await this.contract.methods.setPrice(sendPrice).estimateGas({from: this.account})
                await this.contract.methods.setPrice(sendPrice).send({from: this.account, gas: fee})
                .then(receipt => {
                    this.setting = false
                    console.log('receipt===========>', receipt)
                    this.popup("Price saved", 'Success')
                });
            } catch (err) {
                this.setting = false
                console.log("err", err)
                this.popup("You can not set the price", 'Error')
            }  
        },
        showErrorMessage (err) {
            ...
            if (err.message.indexOf('SenderNotExist') > -1) { 
                ...
                text = "Sorry you didn't invite that account"
            } else if (err.message.indexOf('SubInvitedUserExist') > -1) {
                text = "Sorry that account can't be deleted"
            } else if (err.message.indexOf('AccountNotExist') > -1) {
                text = "Sorry that account does not exist"
            } else if (err.message.slice(0, err.message.indexOf("\n")) != undefined &&
                        err.message.slice(0, err.message.indexOf("\n")) != "") {
                text = err.message.slice(0, err.message.indexOf("\n"));
            }
            this.popup(text, 'Error')
        }          
    }
